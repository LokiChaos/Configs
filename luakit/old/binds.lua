...
add_binds("normal", {
    ...
    buf("^gr$",                     function (w, b, m) w:scroll{ ypct = m.count } w:reload()     end, {count = 0}),
    buf("^gR$",                     function (w, b, m) w:scroll{ ypct = m.count } w:reload(true) end, {count = 0}),

    -- Match my vim binding to unhighlight search matches
    key({"Control"}, "space",       function (w) w:clear_search() end),
    ...
    -- NEW, toggle visibility of tab bar and status bar
    key({},          "F10",         function (w)
                                        w.sbar.ebox.visible = not w.sbar.ebox.visible
...
add_cmds({
 ...
 -- cmd("co[mmand]",            function (w, arg, opts) .. end, opts),
    ...
    cmd("reloadall",            "reload all tabs",
                                function (w) w:reloadall() end),
